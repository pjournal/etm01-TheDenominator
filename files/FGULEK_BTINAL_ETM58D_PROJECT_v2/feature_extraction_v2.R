...
extract_features.openclose <- function(matches,odd_details,pMissThreshold=0.01,trainStart,testStart){
  ...
  details = dcast(feature_odd_details,matchId~oddtype+bookmaker,value.var = c("Odd_Close"))
  columnsToKeep = grep(paste(bookiesToKeep,collapse="|"),names(details),value=T)
  details = details[,c('matchId',columnsToKeep),with=F]
  #HANDLE MISSINGS END
  
  
  details = merge(matches[,-c('Home','Away','Match_Result','Result_Home','Result_Tie','Result_Away','type'),with=F],
                  details,by="matchId",all=T)
  
  details[,outcome:=ifelse(Home_Score>Away_Score,"home_win",
                                 ifelse(Home_Score==Away_Score,"draw", "away_win"))]
